...
(function () {
  ...
  function normalizeArabic(text) {
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      if (cp >= 0x0610 && cp <= 0x061A || cp >= 0x064B && cp <= 0x065F || cp === 0x0670 || cp === 0x0640) continue;
  ...
  function checkArabicPronunciation(expected, heard) {
    ...
    if (nh.includes(ne) || ne.includes(nh)) return {
      correct: true,
      heard: heard
    };
    ...
    return {
      correct: levenshtein(ne, nh) <= threshold,
      heard: heard
    };
  }
  ...
  function startListeningArabic(onResult, onError) {
    ...
    _recognition.onresult = function (e) {
      _gotResult = true;
      var results = Array.from(e.results[0]).map(function (r) {
        return r.transcript;
      });
