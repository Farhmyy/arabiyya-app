...
const SRS = function () {
  ...
  function scheduleCard(card, rating) {
    ...
    if (q < 3) {
      ...
      if (repetitions === 0) newInterval = 1;else if (repetitions === 1) newInterval = 6;else newInterval = Math.round(interval * easeFactor);
  ...
  function getDueCards(srsCards) {
    ...
    return Object.keys(srsCards).map(Number).filter(i => {
      const c = srsCards[i];
