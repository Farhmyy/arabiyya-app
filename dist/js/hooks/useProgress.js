  ...
  chapters: {
    '3': {
      hiwar: {
        completed: false,
        score: 0,
        maxScore: DATA?.hiwar?.scenes?.length ?? 6
      },
      mufrodat: {
        completed: false,
        score: 0,
        maxScore: DATA?.mufrodat?.length ?? 6
      },
      tadribat_1: {
        completed: false,
        score: 0,
        maxScore: DATA?.tadribat_1?.questions?.length ?? 15,
        bestScore: 0,
        attempts: 0
      },
      qawaid: {
        completed: false,
        score: 0,
        maxScore: DATA?.qawaid?.topics?.length ?? 3
      },
      tadribat_2: {
        completed: false,
        score: 0,
        maxScore: DATA?.tadribat_2?.questions?.length ?? 10,
        bestScore: 0,
        attempts: 0
      },
      imtihan: {
        completed: false,
        score: 0,
        maxScore: DATA?.imtihan?.questions?.length ?? 15,
        bestScore: 0,
        attempts: 0
      }
    }
...
function useProgress() {
  ...
  const completeSection = useCallback((chapterId, sectionId, score, maxScore, wrongIndices) => {
    persist(prev => {
      ...
      const srsCards = prev_sec.srsCards || (sectionId === 'mufrodat' && window.SRS ? window.SRS.initSrsCards(maxScore) : null);
      chapters[chapterId] = {
        ...chapters[chapterId],
        [sectionId]: {
          completed: true,
